...
import { Search, TrendingUp, TrendingDown, ArrowLeft, LayoutDashboard } from "lucide-react";
...
const Assets = () => {
  ...
  const categories = ["Todos", ...Array.from(new Set(holdings.map((h) => h.category)))];

  const filtered = holdings.filter((h) => {
    const matchSearch =
      h.symbol.toLowerCase().includes(search.toLowerCase()) ||
      h.name.toLowerCase().includes(search.toLowerCase());
    const matchCategory = categoryFilter === "Todos" || h.category === categoryFilter;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Explore todos os ativos da sua carteira</p>
        </div>
          ...
          {filtered.map((asset) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent text-muted-foreground">{asset.category}</span>
              </div>
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {asset.shares} {asset.shares === 1 ? "unidade" : "unidades"}
                  </p>
                </div>
